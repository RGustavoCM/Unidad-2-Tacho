...
class Address:
    """__init__() funcion como constructor"""
...
class AddressBook:
    ...
    def initialization(self):
        ...
        self.status_bar = Label(main_frame, text="Ramiro Gustavo -- 2020", relief=SUNKEN, bd=1).pack(side=BOTTOM, fill=X)
        ...
        self.btn_new = ttk.Button(frame4, text='Nuevo', command=self.on_new, width=10)
        self.btn_new.pack(side=LEFT)
        self.btn_add = ttk.Button(frame4, text='Agregar', command=self.on_add, width=10)
        self.btn_add.pack(side=LEFT)
        self.btn_mod = ttk.Button(frame4, text='Modificar', command=self.on_mod, width=10)
        self.btn_mod.pack(side=LEFT)
        self.btn_del = ttk.Button(frame4, text='Eliminar', command=self.on_del, width=10)
    ...
    def save_address(self):
        ...
        tkinter.messagebox.showinfo("Contacto Guardado", "Nuevo ELemento Agregado")
        outfile.close()

    def modify_address(self):
        outfile = open("address.dat", "wb")
        pickle.dump(self.lst_addresses, outfile)
        tkinter.messagebox.showinfo("Contacto Modificado", "The changes have been saved")
        outfile.close()

    def delete_address(self):
            outfile = open("address.dat", "wb")
            pickle.dump(self.lst_addresses, outfile)
            tkinter.messagebox.showinfo("Contacto ELiminado", "EL Contacto Ha Sido Eliminado")
            outfile.close()
...
if __name__ == '__main__':
    ...
    application = AddressBook(root, "Ejercicio 22")
